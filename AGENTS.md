...
## 项目结构与模块划分
仓库采用 `apps/` + `engine/` 结构。`apps/macos/CodexIsland/` 是当前 macOS 主应用目录，其中 `App/` 放应用入口与窗口生命周期，`Core/` 放状态、几何与设置等核心逻辑，`Events/` 处理事件监听，`Models/` 定义数据模型，`Services/` 按能力拆分为 `Chat/`、`Hooks/`、`Session/`、`Tmux/`、`Update/`、`Window/` 等模块。界面代码位于 `UI/Views` 和 `UI/Components`，窗口封装在 `UI/Window`。资源文件在 `Assets.xcassets/`、`Resources/` 与 `Info.plist`。`engine/` 是共享 Rust 引擎 workspace，承接协议、运行时、host daemon 与后续 FFI。发布脚本仍在 `scripts/`。
...
## 安全与配置提示
禁止提交 `.sparkle-keys/`、公证凭据或本地 keychain 配置。修改 `apps/macos/CodexIsland/Services/Hooks/` 或 `apps/macos/CodexIsland/Resources/codex-island-state.py` 时要格外谨慎，这些内容会直接影响用户本地 Codex hooks 的安装与运行。
