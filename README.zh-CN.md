...
- 聊天历史现在支持图片附件展示，本地和远端会话都能还原并显示图片内容。
- 远端 thinking 历史会过滤空白项，聊天记录更干净。
- hooks 安装更安全：既修复 `config.toml` 错误重写问题，也保留启用 hooks 时的原始换行格式。
- 本地停止会话的清理和 notch 会话绑定逻辑更准确，减少残留状态和误识别。
- 启用 `Remote Debug Logs` 后，聊天头部会显示可复制的 session/thread ID，方便排查问题。
- GitHub Actions 的 Swift 质量工作流现在会显式选择稳定 Xcode，并隔离 DerivedData 目录来执行构建和测试。
...
当前实现仍然是以 macOS 进程内的 hooks 流程为主。仓库里的 `engine/` workspace 是共享 Rust 引擎基础，后续会承接 transcript 解析、状态聚合、host daemon 与跨平台 FFI 等职责。

## 项目结构

- `apps/macos/CodexIsland/App/`：应用生命周期和窗口启动
- `apps/macos/CodexIsland/Core/`：设置、几何计算、屏幕选择等基础能力
- `apps/macos/CodexIsland/Services/`：hooks、本机会话解析、远端 app-server 管理、tmux 集成、更新、窗口管理
- `apps/macos/CodexIsland/UI/`：刘海视图、菜单界面、聊天界面和复用组件
- `apps/macos/CodexIsland/Resources/`：随应用分发的脚本资源，例如 `codex-island-state.py`
- `scripts/`：构建、签名、公证、发布辅助脚本
- `engine/`：共享 Rust workspace，承接协议、运行时、host daemon 与绑定
...
`./scripts/swift-quality.sh` 会在同一轮里同时检查 `apps/macos/CodexIsland/` 和 `apps/macos/CodexIslandTests/`。`./scripts/install-git-hooks.sh` 会把 Git 切到仓库内的 `.githooks/` 包装层，先保留 beads 现有 hooks，再在 `pre-commit` 里追加已暂存 Swift 文件的质量检查，避免历史质量债阻塞无关提交。
...
如果改动了 `apps/macos/CodexIsland/Services/Hooks/` 或 `apps/macos/CodexIsland/Resources/codex-island-state.py`，要把它视为会直接影响用户本地 Codex 环境的高风险改动，务必手工验证。
