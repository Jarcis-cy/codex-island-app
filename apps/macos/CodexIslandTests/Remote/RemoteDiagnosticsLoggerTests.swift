...
final class RemoteDiagnosticsLoggerTests: XCTestCase {
    ...
    func testLoggerReportsWriteFailure() async throws {
        ...
        let recorder = LockedValueBox([String]())
        let logger = RemoteDiagnosticsLogger(
            ...
            reportFailure: { message in
                recorder.withValue { $0.append(message) }
            }
        )
        ...
        let recordedMessages = recorder.get()
        XCTAssertEqual(recordedMessages.count, 1)
        XCTAssertTrue(recordedMessages.first?.contains("Failed to persist remote diagnostics") == true)
    }
}
